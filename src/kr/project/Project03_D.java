...
public class Project03_D {
	...
	private static void getIsbnImage(ExcelVO vo) {
		...
		try {
			...
			con.setRequestProperty("X-Naver-Client-Id", "veDW3iy2hKjiSbMRDxgK");
			con.setRequestProperty("X-Naver-Client-Secret", "PPhoYGA2GH");
			...
			if(!total.text().equals("0")) {
				...
				//imgDoc.indexOf("<img>") + 5 : 앞에서부터 읽되 <img>란 글자 제외
				String imgTag = imgDoc.substring(imgDoc.indexOf("<img>") + 5);
				String imgURL = imgTag.substring(0, imgTag.indexOf("?")); //처음부터 ?전까지
